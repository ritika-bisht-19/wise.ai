    ...
    >
      <ul className={listClassName}>
        {repeatedItems.map((item, index) => (
          <li key={`${item}-${index}`} className="shrink-0 flex items-center gap-2 md:gap-3">
            <span className="font-matter font-bold text-[11px] md:text-[14px] tracking-[0.24em] uppercase text-white/95 whitespace-nowrap">
              {item}
            </span>
            <span aria-hidden="true" className="mx-2 md:mx-3 inline-block w-2 h-2 rounded-full bg-[#0D5AE5]" />
          </li>
        ))}
      </ul>
    </div>
    ...
    <section className="relative flex flex-col pt-28 md:pt-40 min-h-screen overflow-visible">
      ...
      <div className="flex flex-1 justify-center items-center mx-auto pb-[12vh] w-[85%] md:w-9/12 max-w-[1280px] overflow-visible">
        <div className="relative flex flex-col items-center">
          <img src="/assets/images/hero/hero-gradient-bg.svg" alt="" className="top-[-80%] md:top-[-165%] left-1/2 absolute w-[160%] md:w-[220%] max-w-none h-auto scale-x-200 scale-y-170 -translate-x-1/2 pointer-events-none" />
          ...
          <div className="z-10 relative flex flex-col items-center gap-5 md:gap-10">
            <div className="mt-16 md:mt-24 flex flex-col items-center gap-4 md:gap-5">
              <img src="/assets/images/hero/hero-motif.svg" alt="" role="presentation" className="w-auto h-10 object-cover animate-float" />
